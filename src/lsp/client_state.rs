...
use super::Stop;
use crate::lsp::{IndexingProgress, IndexingStage, LspNotification};
...
impl LanguageClient for ClientState {
    ...
    fn progress(&mut self, params: ProgressParams) -> Self::NotifyResult {
        ...
        let is_indexing = matches!(params.token, NumberOrString::String(ref s) if RA_INDEXING_TOKENS.contains(&s.as_str()));
...
impl ClientState {
    fn parse_progress(&self, params: &ProgressParams) -> Option<IndexingProgress> {
        ...
        let crate_regex = CRATE_REGEX
            .get_or_init(|| Regex::new(r"(?:indexing|building|loading)\s+([\w-]+)").unwrap());
        let count_regex = COUNT_REGEX.get_or_init(|| Regex::new(r"(\d+)/(\d+)").unwrap());
        ...
        let stage = match &params.token {
            ...
            NumberOrString::Number(n) => IndexingStage::Unknown(format!("numeric_{}", n)),
        };
